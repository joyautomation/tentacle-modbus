...
export const log: Record<string, Log> = {
  service: createLogger("modbus", currentLevel),
  modbus: createLogger("modbus:modbus", currentLevel),
  nats: createLogger("modbus:nats", currentLevel),
};
...
/**
 * Wrap all exported loggers to also publish to NATS.
...
export function enableNatsLogging(
  ...
  for (const key of Object.keys(log)) {
    log[key] = wrapLogger(log[key], publishFn, `modbus:${key}`);
  }
}
